...
export default function Footer() {
    ...
    const footerStyle = `font-light text-[1rem] w-[100vw] bg-primary10 backdrop-blur-md text-customWhite py-4 bottom-0  ${!(isOpen || isATE) && "translate-y-[calc(100%-17px)]"}  flex flex-col items-center justify-end shadow-[0px_0px_15px_rgba(0,0,0,0.3)] px-8 fixed duration-300 z-50 rounded-t-lg`

    const arrowStyle = `duration-150 ease-linear ${isOpen || isATE ? "-rotate-90" : "rotate-90"} `
    const sectionStyle = "flex w-full justify-between [&>*]:w-1/3"
    ...
    return (
        ...
        >
            <span className='p-1 bg-secondary rounded-full absolute -top-2 shadow-sm'>
